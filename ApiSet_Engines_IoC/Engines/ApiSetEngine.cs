...
{
    public class ApiSetEngine : IApiSetEngine
    {
        #region -- Properties -----
        private readonly ISettings _settings;
        private readonly ILogger _logger;
        private readonly IControllerEngine _controllerEngine;
        private readonly IComponentEngine _componentEngine;
        private readonly IEndpointEngine _endpointEngine;

        public ApiDoc apiDoc { get; set; }
        #endregion

        #region -- Constructors -----
        public ApiSetEngine(ILogger logger
                            , ISettings settings
                            , IControllerEngine controllerEngine
                            , IComponentEngine componentEngine
                            , IEndpointEngine endpointEngine)
        {
            _settings = settings;
            _logger = logger;
        ...
        private void SetExtraInfo(string apiRootFromSettings, Dictionary<string, string> extraInfo)
        {
            if (extraInfo.ContainsKey("basePath"))
            {
                if (extraInfo["basePath"] != "")
                {
                    ...
                    apiDoc.apiRootSourceLocation = ApiTestGenerator.Models.Enums.ApiRootSourceEnum.basePath;
                    return;
                }
            }
